...
const DEFAULT_MAX_FILE_SIZE = 100 * 1024 * 1024; // 5MB
...
export default function FileUpload({
    ...
    console.log("Uploaded"), setFileExist(true), setDocUrl(file.publicUrl)},
    onUploadError = () => {},
    ...
    const uploadToSupabase = useCallback(async (uploadingFile) => {
    setStatus("uploading");
    setProgress(0);

    const filePath = `uploads/${Date.now()}_${uploadingFile.name}`;
    ...
    try {
        ...
        if (uploadError) {
            handleError({ message: uploadError.message, code: uploadError.code });
            return; 
        }
        setProgress(90);
        ...
        if (urlError) {
            handleError({ message: urlError.message, code: urlError.code });
            return;
        }

        const docId = uuidv4();
        const res = await axios.post(`${server_url}/api/send`, {
            uploadurl: urlData.publicUrl,
            docId: docId
        });

        if(res.status == 500){
           return handleError({ message: "Document processing failed" });
        }

        setProgress(100);

        localStorage.setItem("currentDoc", docId);
        localStorage.setItem("currentDocName", filePath);
        ...
        handleError({ message: errorMessage, code: "UPLOAD_OR_PROCESS_ERROR" });
    }
}, [uploadDelay, onUploadSuccess, handleError]);
    ...
    return (
        (<div
            className={cn("relative w-[550px] mx-auto", className || "")}
            role="complementary"
                                ...
                                {status === "idle" || status === "dragging" ? (
                                    <motion.div
                                        ...
                                        <button
                                            ...
                                            className="cursor-pointer w-4/5 flex items-center justify-center gap-2 rounded-lg bg-[#699dfe] dark:bg-white/10 px-4 py-2.5 text-sm font-semibold text-gray-900 dark:text-white transition-all duration-200 hover:bg-gray-200 dark:hover:bg-white/20 group">
